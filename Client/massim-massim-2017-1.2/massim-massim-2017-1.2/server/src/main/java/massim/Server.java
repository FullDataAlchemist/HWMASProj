...
public class Server
{
	...
	private void go()
	{
		...
		switch (config.logLevel)
		{
			...
			case "error":
				...
				break;
			case "critical":
				Log.setLogLevel(Log.Level.CRITICAL);
				break;
			default:
				Log.setLogLevel(Log.Level.NORMAL);
		}
			...
			case ServerConfig.MODE_ROUND_ROBIN:
				...
				while (nextMatch)
				{
					...
					for (int i = indices.length - 1; i >= 0; i--)
					{
						...
						if (i == 0)
							nextMatch = false; // no team constellation left
					}
				}
				break;
			case ServerConfig.MODE_MANUAL:
				if (config.manualModeTeams != null)
					config.manualModeTeams.forEach(this::runMatch);
				break;
	 ...
	 *
	 * @param matchTeams
	 *            a set of all teams to participate in the simulation
	 */
	 ...
	 *
	 * @param sim
	 *            the simulation that may receive some of the commands
	 */
	 ...
	 *
	 * @param command
	 *            the already split command string
	 */
	 ...
	 *
	 * @param simID
	 *            the ID of the current sim
	 * @param startTime
	 *            string representation of the simulation's start time
	 * @param world
	 *            the world state
	 */
	...
	{
		if (monitor != null)
			monitor.updateState(world);
		if (config.replayPath != null)
		{
			// determine file name
			String file = "";
			if (world instanceof StaticWorldData)
				file = "static";
			else if (world instanceof DynamicWorldData)
				file = "step-" + String.valueOf(((DynamicWorldData) world).step);
			// save to file
	 ...
	 *
	 * @param conf
	 *            the JSONObject (configuration) holding a "server" JSONObject
	...
	public static ServerConfig parseServerConfig(JSONObject conf)
	{
		...
		if (teamJSON == null)
			Log.log(Log.Level.ERROR, "No teams configured.");
		else
		{
				...
				for (int i = 0; i < manualConf.length(); i++)
				{
					...
					if (teamList != null)
					{
						...
						for (int j = 0; j < config.teamsPerMatch; j++)
						{
							...
							if (teamConfig != null)
								parsedTeamNames.add(teamMap.get(team));
							else
								Log.log(Log.Level.ERROR, "No team with name " + team + " configured.");
						}
						if (parsedTeamNames.size() == config.teamsPerMatch)
							matchTeams.add(parsedTeamNames);
					}
				}
